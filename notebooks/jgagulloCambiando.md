...
folder = f"//imgserver.cnio.es/IMAGES/CONFOCAL/IA/Projects/2024/2024_10_21_jgagullo/spatial analisis/qpath/data/{cnio_user}"
...
for file in os.listdir(folder):
    if file.lower().endswith('.tiff') or file.lower().endswith('.tif'):
        ...
        # Add to the dictionary using only the filename as the key
        images_dict[file] = image

# Display a summary of the loaded images
print(f"{len(images_dict)} images were loaded.")
...
```python
# Path to the folder
folder_path = fr"\\imgserver.cnio.es\IMAGES\CONFOCAL\IA\Projects\2024\2024_10_21_jgagullo\spatial analisis\qpath\qpathprojects\{cnio_user}\export_DAPI"
...
# Pattern to extract 'roiX_dapi' from the filename
pattern = r"(roi\d+_dapi)"

# Process each file in the folder
for file_name in tif_files:
    # Extract the desired name using the regular expression
    match = re.search(pattern, file_name.lower())
    if not match:
        print(f"The file '{file_name}' does not match the expected pattern.")
        continue

    extracted_name = match.group(1)  # Get the match
    file_path = os.path.join(folder_path, file_name)
    
    # Load the labeled mask
    try:
        mask = imageio.imread(file_path)
        dapi_masks_dict[extracted_name] = mask
        print(f"Processing: {extracted_name} - Dimensions: {mask.shape}")
...
ck_channel_index = 10  # Index of the CK channel (adjust if needed)



user_score = -1  # Score provided by the user (value between 0 and 10)
base_folder_path = fr"\\imgserver.cnio.es\IMAGES\CONFOCAL\IA\Projects\2024\2024_10_21_jgagullo\spatial analisis\qpath\qpathprojects\{cnio_user}\exportedmasks"


min_size = 2000  # Minimum size of objects to retain (adjust as needed)
max_hole_size = 10000  # Maximum size of holes to fill

# Map the score (0-10) to the range (0-3)
scaling_factor = (user_score / 10) * 3

# Dictionary to store CK masks with the ROI name as the key
ck_masks_dict = {}
...
def extract_roi_number(filename):
    ...
    return None
"""
def post_process_mask(mask, min_size=1000, max_hole_size=75000):
...
"""
for name, image in images_dict.items():
    # Extract the ROI number from the image name
    roi_number_image = extract_roi_number(name)

    if roi_number_image is None:
        print(f"Could not extract the ROI number from the image {name}. Skipping...")
        continue

    # Extract the CK channel from the image
    ck_channel = image[ck_channel_index, ...].astype(float)

    # Calculate the mean and standard deviation of the CK channel
    mean_intensity = np.mean(ck_channel)
    std_intensity = np.std(ck_channel)

    # Calculate the threshold as: mean + scaling_factor * standard deviation
    threshold = mean_intensity + scaling_factor * std_intensity

    # Create the binary mask for the CK channel
    ck_mask = ck_channel > threshold  # This is the original mask for the CK channel

    # Get the corresponding DAPI mask using the ROI number
    roi_mask_key = f'roi{roi_number_image}_dapi'  # Key name for the DAPI mask
    dapi_mask = dapi_masks_dict.get(roi_mask_key, None)

    
    final_mask = ck_mask  # Overwrite to use only CK, if necessary

    # Post-process the mask (e.g., removing small objects, filling holes)
    mask_post_processed = post_process_mask(final_mask, min_size=min_size, max_hole_size=max_hole_size)

    # Store the post-processed mask in the ck_masks_dict with the '_ck' suffix
    roi_key = f'roi{roi_number_image}'  # Key for the dictionary
    ck_masks_dict[f"{roi_key}"] = mask_post_processed  # <== Changed here

    # Save the processed mask in the output folder
    folder_name = f"{name} - ROI{roi_number_image}"
    roi_folder = os.path.join(base_folder_path, folder_name)
    if not os.path.exists(roi_folder):
        os.makedirs(roi_folder)

    mask_filename = "CK_mask_binary.tif"
    mask_filepath = os.path.join(roi_folder, mask_filename)
    tifffile.imwrite(mask_filepath, mask_post_processed.astype(np.uint8))
    print(f"Mask saved at: {mask_filepath}")

    # Create figure for the plot (optional)
    fig, axs = plt.subplots(1, 3, figsize=(20, 10))

    # Plot 1: Original CK channel
    axs[0].imshow(ck_channel, cmap='gray')
    axs[0].set_title(f'{name}: CK Channel (ch {ck_channel_index})')
    axs[0].axis('off')

    # Plot 2: Original binary mask (using ck_mask instead of mask)
    axs[1].imshow(final_mask, cmap='gray')  # Use ck_mask here
    axs[1].set_title('CK Binary Mask (original)')
    axs[1].axis('off')

    # Plot 3: Post-processed mask
    axs[2].imshow(mask_post_processed, cmap='gray')
    axs[2].set_title(f'Post-Processed Mask\n(min_size={min_size}, max_hole_size={max_hole_size})')
    axs[2].axis('off')
...
# **Verification of Keys in ck_masks_dict**
print("Keys in ck_masks_dict:")
for key in ck_masks_dict.keys():
    print(key)
...
import matplotlib.pyplot as plt
import tifffile
import re  # Ensure re is imported for extract_roi_number
...
# Configuration parameters
ngfr_channel_index = 13  # Index of the NGFR channel (adjust if needed)
base_folder_path = fr"\\imgserver.cnio.es\IMAGES\CONFOCAL\IA\Projects\2024\2024_10_21_jgagullo\spatial analisis\qpath\qpathprojects\{cnio_user}\exportedmasks"
...
min_size = 200  # Minimum size of objects to retain (adjust if needed)
max_hole_size = 75000  # Maximum size of holes to fill

# Define the user scores for each ROI
user_scores = {
    'ROI1.ome.tiff': 2.7,
    'ROI2.ome.tiff': 2.7,
    'ROI3.ome.tiff': 2.7,
...
# Dictionary to store NGFR masks with the ROI name as the key
ngfr_masks_dict = {}

# Define the brightness factor for visualization
brightness_factor = 25  # Adjust this value as needed
...
for name, image in images_dict.items():
    ...
    # Retrieve the user score for the current ROI
    user_score = user_scores.get(name, 8)  # Default to 8 if not specified
    
    # Map the score (0-10) to the range (0-3)
    scaling_factor = (user_score / 3) * 3
    
    # Extract the NGFR channel from the image
    ngfr_channel = image[ngfr_channel_index, ...].astype(float)
    
    # Calculate the mean and standard deviation of the NGFR channel
    mean_intensity = np.mean(ngfr_channel)
    std_intensity = np.std(ngfr_channel)
    
    # Calculate the threshold as: mean + scaling_factor * standard deviation
    threshold = mean_intensity + scaling_factor * std_intensity
    
    # Create the binary mask for the NGFR channel
    ngfr_mask = ngfr_channel > threshold  # This is the original mask for the NGFR channel
    ...
    if dapi_mask is None:
        print(f"DAPI mask for {roi_mask_key} not found. Skipping...")
        continue
    
    # Create the final mask by combining NGFR and DAPI
    final_mask = ngfr_mask & (dapi_mask > 0)  # Only where DAPI has values > 0
    # If you need to use only NGFR, uncomment the following line
    # final_mask = ngfr_mask
    
    # Post-process the mask
    mask_post_processed = post_process_mask_closing(final_mask, min_size=min_size, max_hole_size=max_hole_size)
    mask_post_processed = post_process_mask(mask_post_processed, min_size=10000, max_hole_size=max_hole_size)
    
    # Store the post-processed mask in the ngfr_masks_dict
    roi_key = f'roi{roi_number_image}'  # Key for the dictionary
    ngfr_masks_dict[f"{roi_key}"] = mask_post_processed
    ...
    # Plot 1: Original NGFR channel with brightness enhancement
    ngfr_bright = ngfr_channel * brightness_factor
    ngfr_bright = np.clip(ngfr_bright, 0, np.max(ngfr_channel))
    axs[0].imshow(ngfr_bright, cmap='gray')
    axs[0].set_title(f'{name}: NGFR Channel (ch {ngfr_channel_index})')
    axs[0].axis('off')
    
    # Plot 2: Original binary mask
    axs[1].imshow(final_mask, cmap='gray')
    axs[1].set_title('NGFR Binary Mask (original)')
    axs[1].axis('off')
...
# **Verification of Keys in ngfr_masks_dict**
print("Keys in ngfr_masks_dict:")
for key in ngfr_masks_dict.keys():
    print(key)
...
```python
my_rois = ["roi8"] #,"roi13","roi11","roi5","roi6","roi7"
#my_rois = ["roi13"]
...
PIXEL_SIZE_MICROMETERS = 0.17  # µm
PIXEL_AREA_MICROMETERS2 = PIXEL_SIZE_MICROMETERS ** 2
# Parallelize processing of all ROIs
results = Parallel(n_jobs=-1)(
    delayed(process_roi)(
        ...
        dapi_masks_dict, 
        ck_masks_dict,
        ngfr_masks_dict,  # Asegúrate de pasar ngfr_masks_dict
        channels_of_interest, 
        marker_dict,
        PIXEL_AREA_MICROMETERS2  # Pasar el área del píxel en µm²
    ) 
...
# Identify marker columns (excluding ROI and DAPI_ID)
marker_columns = [col for col in df.columns if col not in ['ROI', 'DAPI_ID','Area_pixels','Area_um2','centroid_x' 	,'centroid_y']]

# Compute mean and std for each ROI
roi_means = df.groupby('ROI')[marker_columns].mean().reset_index()
roi_stds = df.groupby('ROI')[marker_columns].std().reset_index()

# Merge mean and std back to the original DataFrame
df_merged = df.merge(roi_means, on='ROI', suffixes=('', '_roi_mean'))
df_merged = df_merged.merge(roi_stds, on='ROI', suffixes=('', '_roi_std'))

# Example "notes" from 0 to 5 for each marker


# Create binary columns based on threshold = mean + (scaled_note * std)
for marker in marker_columns:
    note = marker_notes.get(marker, 0)
    scaled_note = note #* (3 / 5.0)  # maps [0..5] to [0..3]
    
    mean_col = f'{marker}_roi_mean'
    std_col = f'{marker}_roi_std'
    threshold_col = f'{marker}_threshold'
    
    df_merged[threshold_col] = df_merged[mean_col] + (scaled_note * df_merged[std_col])
    
    binary_col = f'{marker}_binary'
    df_merged[binary_col] = (df_merged[marker] > df_merged[threshold_col]).astype(int)

# Select only ROI, DAPI_ID, and the new binary columns
binary_columns = [f'{marker}_binary' for marker in marker_columns]
df_binary = df_merged[['ROI', 'DAPI_ID','Area_pixels','Area_um2','centroid_x' 	,'centroid_y'] + binary_columns]
...
# 1. Verify the columns and the first few rows of df_binary
print("Columns in df_binary:", df_binary.columns.tolist())
print(df_binary.head())
filtered_df = df_binary[df_binary['ROI'].isin(my_rois)]
...
# 3. Initialize a DataFrame to store the counts
counts_df = pd.DataFrame()

# 4. Group by 'ROI' and calculate counts for each combination
grouped = filtered_df.groupby('ROI')

for name, condition in combinations.items():
    counts = grouped.apply(lambda x: condition(x).sum()).rename(name)
    counts_df = pd.concat([counts_df, counts], axis=1)

# 5. Reset the index and rename 'index' to 'ROI' if necessary
if 'ROI' not in counts_df.columns:
    counts_df = counts_df.reset_index().rename(columns={'index': 'ROI'})
else:
    counts_df = counts_df.reset_index()

# Calculate the total number of cells per ROI using the length of the DataFrame
counts_df['Total Cells'] = grouped.apply(lambda x: len(x)).values

# 6. Print the counts table
print("Counts table of positive cells by combination and ROI:")
print(counts_df)

# 7. Transform the DataFrame for plotting
counts_melted = counts_df.melt(id_vars=['ROI', 'Total Cells'], var_name='Combination', value_name='Count')

# 8. Define the save path correctly
# Construct the path using os.path.join for cross-platform compatibility
path_save = os.path.join("..\\results_spatial_analysis", cnio_user, "cell_counting")

# 9. Check if the directory exists and create it if it doesn't
if not os.path.exists(path_save):
    os.makedirs(path_save)
    print(f"Directory created: {path_save}")
else:
    print(f"Directory already exists: {path_save}")

# 10. Define the base filename for the files
base_filename = "Tumor_Characterization"

# 11. Define the filenames for CSV and SVG
csv_filename = f"{base_filename}.csv"
svg_filename = f"{base_filename}.svg"

# 12. Construct the full file paths
csv_filepath = os.path.join(path_save, csv_filename)
svg_filepath = os.path.join(path_save, svg_filename)

# 13. Save the DataFrame as CSV
counts_df.to_csv(csv_filepath, index=True)
print(f"Table saved as CSV at: {csv_filepath}")

# 14. Create the bar plot
plt.figure(figsize=(20, 10))
ax = sns.barplot(x='ROI', y='Count', hue='Combination', data=counts_melted)

# Add numbers vertically above each bar
for p in ax.patches:
    height = p.get_height()
    if not pd.isnull(height) and height > 0:  # Ensure the height is not null or zero
        ax.text(
            ...
            va='bottom', 
            fontsize=10, 
            rotation=90  # Numbers in vertical orientation
        )

# Add the total number of cells in the x-axis labels
new_labels = [f'{roi}\n(Total: {total})' for roi, total in zip(counts_df['ROI'], counts_df['Total Cells'])]
ax.set_xticklabels(new_labels)

# Set plot title and labels
plt.title('Tumor Characterization: Count of Positive Cells by Combination and ROI', fontsize=16)
plt.xlabel('ROI', fontsize=14)
plt.ylabel('Number of Positive Cells', fontsize=14)
...
# 15. Save the plot as SVG
plt.savefig(svg_filepath, format='svg', bbox_inches='tight')
print(f"Plot saved as SVG at: {svg_filepath}")
...
# Filter df_binary for the specified ROIs
filtered_df = df_binary[df_binary['ROI'].isin(my_rois)]
...
# 10. Define the base filename for the files based on the plot title
base_filename = "Tumor Infiltration"
...
# Set plot title and labels
plt.title('Tumor Infiltration (Immune Cells): Count of Positive Cells by Combination and ROI', fontsize=16)
plt.xlabel('ROI', fontsize=14)
plt.ylabel('Number of Positive Cells', fontsize=14)
plt.legend(title='Combination', fontsize=12, title_fontsize=14)
plt.xticks(rotation=45)
...
# 10. Define the base filename for the files based on the plot title
base_filename = "Tumor Infiltration Related to NGFR"
...
# Set plot title and labels
plt.title('Tumor Infiltration (Immune Cells) Related to NGFR: Count of Positive Cells by Combination and ROI', fontsize=16)
plt.xlabel('ROI', fontsize=14)
plt.ylabel('Number of Positive Cells', fontsize=14)
plt.legend(title='Combination', fontsize=12, title_fontsize=14)
plt.xticks(rotation=45)
...
```python
def compute_mask_area_summary(ck_masks, ngfr_masks, pixel_area=PIXEL_AREA_MICROMETERS2):
    """
...
# ------------------------------------------------
#      PLOT CK & NGFR FOR A GIVEN ROI / SUBPOP
# ------------------------------------------------
...
# ------------------------------------------------
#    MAIN: COMPUTE & SAVE FOR ONE ROI / SUBPOP
# ------------------------------------------------
...
def parse_conditions(conditions_list, condition_column_map):
    """
    Converts a list of condition strings (like ['CK_mask+', 'NGFR_intensity-']) into
    a dictionary mapping the corresponding binary columns to 1 or 0.
    """
    parsed = {}
    for cond in conditions_list:
        ...
        if cond.endswith('+'):
            ...
            print(f"Invalid condition: '{cond}'. Must end with '+' or '-'. Ignored.")
            continue

        if base in condition_column_map:
            parsed[condition_column_map[base]] = val
        else:
            print(f"No mapping found for '{cond}'. Ignored.")
    return parsed

def select_subpopulation(df_roi, parsed_conditions):
    """
    Returns the subset of df_roi matching ALL parsed_conditions (AND logic).
    If parsed_conditions is empty, returns an empty DataFrame by default.
    """
    if len(parsed_conditions) == 0:
        return df_roi.iloc[0:0]

    mask_series = pd.Series([True]*len(df_roi), index=df_roi.index)
    for bin_col, val in parsed_conditions.items():
        if bin_col in df_roi.columns:
            mask_series &= (df_roi[bin_col] == val)
        else:
            mask_series &= False

    return df_roi[mask_series]
...
ruta_base = r'\\imgserver.cnio.es\IMAGES\CONFOCAL\IA\Projects\2024\2024_10_21_jgagullo\spatial analisis\results_spatial_analysis\jgagullo\distances_between_populations'
dic_distancias = {}
...
save_path = os.path.join("..", "results_spatial_analysis", cnio_user, "boxplots_distances_between_populations")

os.makedirs(save_path, exist_ok=True)

population_dict = {}

for subfolder, files in dic_distancias.items():
    for file_name, df in files.items():
        # Extract ROI, sign (+/-), and population name
        match = re.match(r"(distance_matrix_(roi\d+)_.*NGFR_intensity)([\+\-])(_vs.*)\.csv", file_name)
        if not match:
            continue
        ...
        population = (match.group(1)
                      .replace(f"distance_matrix_{roi}_", "")
                      + match.group(4))

        # Convert to numeric array and filter out NaNs
        arr = pd.to_numeric(df.values.flatten(), errors='coerce')
        arr = arr[~np.isnan(arr)]

        if population not in population_dict:
            population_dict[population] = {}
        if roi not in population_dict[population]:
            population_dict[population][roi] = {}

        if sign == '+':
            population_dict[population][roi]['plus'] = arr
        else:
            population_dict[population][roi]['minus'] = arr

# Colors for plus and minus groups
color_plus = 'lightgreen'
color_minus = 'lightcoral'

for population, rois_data in population_dict.items():
    sorted_rois = sorted(rois_data.keys(), key=lambda x: int(re.findall(r'\d+', x)[0]))
    ...
    for i, roi in enumerate(sorted_rois):
        roi_data = rois_data[roi]
        if 'plus' in roi_data and 'minus' in roi_data:
            plus_data = roi_data['plus']
            minus_data = roi_data['minus']

            pos_plus.append(3*i + 1)
            data_plus.append(plus_data)
            pos_minus.append(3*i + 2)
            data_minus.append(minus_data)

    # Boxplot NGFR+
    bp_plus = ax.boxplot(data_plus, positions=pos_plus, patch_artist=True,
                         widths=0.6, showfliers=False)
    for box in bp_plus['boxes']:
        box.set(facecolor=color_plus, alpha=0.5)

    # Boxplot NGFR−
    bp_minus = ax.boxplot(data_minus, positions=pos_minus, patch_artist=True,
                          widths=0.6, showfliers=False)
    for box in bp_minus['boxes']:
        box.set(facecolor=color_minus, alpha=0.5)

    # Violin plot NGFR+
    vp_plus = ax.violinplot(data_plus, positions=pos_plus, widths=0.6,
                            showmeans=False, showmedians=False, showextrema=False)
    for body in vp_plus['bodies']:
        body.set_facecolor(color_plus)
        body.set_edgecolor('black')
        body.set_alpha(0.5)

    # Violin plot NGFR−
    vp_minus = ax.violinplot(data_minus, positions=pos_minus, widths=0.6,
                             showmeans=False, showmedians=False, showextrema=False)
    for body in vp_minus['bodies']:
        body.set_facecolor(color_minus)
        body.set_edgecolor('black')
        body.set_alpha(0.5)

    # Centered X-axis labels for ROIs
    centers_x = []
    labels = []
    for i, roi in enumerate(sorted_rois):
        if 'plus' in rois_data[roi] and 'minus' in rois_data[roi]:
            x_center = (3*i + 1 + 3*i + 2)/2
    ...
    ax.set_ylabel("Distance [µm]")
    match_vs = re.search(r'vs_(.*)', population)
    if match_vs:
        pop_name = match_vs.group(1).replace("_", " ")  # Replace underscores with spaces for readability
    else:
        pop_name = population  # Default fallback

    # Set the title with the new format
    ax.set_title(f"Distances between tumor cells and {pop_name}")
    # Legend
    plus_patch = Patch(facecolor=color_plus, edgecolor='black', alpha=0.5, label='NGFR+')
    minus_patch = Patch(facecolor=color_minus, edgecolor='black', alpha=0.5, label='NGFR−')
    ax.legend(handles=[plus_patch, minus_patch], loc='upper right')

    plt.tight_layout()

    # Save figure as SVG
    cleaned_population_name = re.sub(r'[^\w\-_\. ]', '_', population)  # Remove invalid characters
    figure_path = os.path.join(save_path, f"Distances between tumor cells and {pop_name}.svg")
    plt.savefig(figure_path, format='svg')  # Save as SVG
...
# Invertir el diccionario: de population_dict (clave = población) a roi_dict (clave = ROI)
roi_dict = {}
for population, rois in population_dict.items():
    for roi, data in rois.items():
        if roi not in roi_dict:
            roi_dict[roi] = {}
        roi_dict[roi][population] = data
...
# Graficar para cada ROI (eje x: poblaciones)
for roi, pop_data in roi_dict.items():
    sorted_pops = sorted(pop_data.keys())  # Ordena las poblaciones (se puede ajustar el criterio)
    
    # Ajustar tamaño de la figura para que los ejes se vean mejor
    fig, ax = plt.subplots(figsize=(10, 6))
    
    pos_plus, data_plus = [], []
    pos_minus, data_minus = [], []
    
    for i, population in enumerate(sorted_pops):
        data = pop_data[population]
        if 'plus' in data and 'minus' in data:
            pos_plus.append(3 * i + 1)
            data_plus.append(data['plus'])
            pos_minus.append(3 * i + 2)
            data_minus.append(data['minus'])
    ...
    # Configurar etiquetas del eje X con las poblaciones, rotadas 90°
    centers_x, labels = [], []
    for i, population in enumerate(sorted_pops):
        center = (3 * i + 1 + 3 * i + 2) / 2
        centers_x.append(center)
    ...
    ax.set_ylabel("Distance [µm]")
    ax.set_title(f"Distances between tumor cells and populations for ROI {roi}")
    ...
    plt.tight_layout()
    cleaned_roi = re.sub(r'[^\w\-_\. ]', '_', roi)
    figure_path = os.path.join(save_path, f"Distances between tumor cells and populations for ROI {cleaned_roi}.svg")
...
## Qué pasa en las zonas de interes
